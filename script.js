...
class AudioEngine {
    constructor() { this.ctx = null; this.masterGain = null; this.sources = {}; this.activeNodes = {}; this.isMuted = false; }
    init() { if (this.ctx) return; this.ctx = new (window.AudioContext || window.webkitAudioContext)(); this.masterGain = this.ctx.createGain(); this.masterGain.connect(this.ctx.destination); }
    ...
    async fetchAudio(url) {
        ...
        } catch (e) { console.error("音檔載入失敗:", url); return null; }
    }
    ...
    async toggleSound(id, url, targetVolume, isActive) {
        ...
        if (isActive) {
            if (!this.sources[id]) this.sources[id] = await this.fetchAudio(url);
            if (!this.sources[id]) return;
            const source = this.ctx.createBufferSource();
            source.buffer = this.sources[id];
            source.loop = true;
    ...
    updateVolume(id, value) { if (this.activeNodes[id]) this.activeNodes[id].gain.gain.setTargetAtTime(value, this.ctx.currentTime, 0.3); }
    stopAll() { Object.keys(this.activeNodes).forEach(id => this.toggleSound(id, null, 0, false)); }
    toggleMasterMute() { if (!this.masterGain) return false; this.isMuted = !this.isMuted; this.masterGain.gain.setTargetAtTime(this.isMuted ? 0 : 1, this.ctx.currentTime, 0.5); return this.isMuted; }
}
...
async function callGemini(userInput, type = 'user') {
    ...
    let thinkingText = "紳士貓正專注地傾聽";
    if (type === 'welcome') promptText = `(系統通知: 訪客剛進入此地。請以伴隨者身份說句溫和的招呼語。)`;
    ...
    try {
        const response = await fetch('/api/chat', { // 連向 Vercel 後端
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: promptText || "您好" })
        });
        const data = await response.json();
        setThinking(false);
        return data.text || "我就在這兒陪著您。";
    } catch (e) {
        setThinking(false);
        return "紳士貓正在休息，晚點再聊吧。";
    }
}
...
function renderSceneTabs() {
    ...
    Object.keys(CONFIG.scenes).forEach(key => {
        ...
        btn.onclick = (e) => { e.stopPropagation(); switchScene(key); };
...
window.onload = async () => {
    ...
    for(let i=0; i<144; i++) { const cell = document.createElement('div'); cell.className = 'grid-cell'; grid.appendChild(cell); }
    renderSceneTabs();
